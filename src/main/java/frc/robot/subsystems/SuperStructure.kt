...
object SuperStructure {
    ...
    val goToScoreCoralWhenClose by command {
        Commands.sequence(waitUntil { Chassis.isWithinGoal(1.5) }, goToSelectedLevel)
    }
    ...
    val goToNetWhileAligning by command {
        waitUntil { Chassis.isWithinGoal(1.75) }.andThen(smartGoTo(RobotState.AlgaeNet))
    }

    val retrieveAlgaeFromReef by command {
        Intake.intakeAlgae.andThen(
    ...
    val autoScoreAlgaeInNet by command {
        smartGoTo(RobotState.AlgaeNet)
            .andThen(
                waitUntil { atPosition },
                Intake.scoreAlgae,
                smartGoTo(RobotState.NewCoralStation),
            )
