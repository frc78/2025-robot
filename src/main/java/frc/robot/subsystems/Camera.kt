...
class Camera(name: String, pose: Transform3d) {
    ...
    //        return
    // (lastEstimatedPose?.timestampSeconds?.minus(Timer.getFPGATimestamp())?.milliseconds) ?:
    // Double.MAX_VALUE.milliseconds
    //    }
    ...
    private fun updateStds(pose: EstimatedRobotPose?, targets: List<PhotonTrackedTarget>) {
        ...
        if (nTargets == 0) {
            ...
            //                currentStds = VecBuilder.fill(Double.MAX_VALUE, Double.MAX_VALUE,
            // Double.MAX_VALUE)
